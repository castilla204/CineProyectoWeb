...
{
    private readonly string nameFileLogs = "ApiPeliculasLog.txt";
    private readonly string pathFileLogs;
    ...
    public LogErrores()
    {
       var RutadirectorioLogs = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Logs");//se crea la ruta del directorio que se llamara Logs donde estos se almacenaran en la raiz del contendor en este caso
        ...
        pathFileLogs = Path.Combine(RutadirectorioLogs, nameFileLogs); //crea la ruta completa ahora con el fichero incluido donde se guardaran los logs
    }

    public void LogError(string message)
    {
        string logMessage = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} error: {message}\n";
        File.AppendAllText(pathFileLogs, logMessage);
    }
}}